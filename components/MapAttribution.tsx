...
export default function MapAttribution() {
  ...
  return (
    <div className="pointer-events-auto absolute bottom-4 left-4 z-10">
      {open && (
        <div className="panel-elevated mb-2 w-max max-w-[220px] rounded-xl px-3 py-2 text-[10px] leading-snug text-[var(--ink-soft)] dark:text-[var(--ink-muted)]">
      ...
      <button
        ...
        aria-expanded={open}
        className="panel-elevated flex h-9 w-9 items-center justify-center rounded-full text-xs font-semibold text-[var(--ink-soft)] hover:text-[var(--ink)] sm:h-7 sm:w-7 dark:text-[var(--ink-muted)] dark:hover:text-[var(--accent-deep)]"
      >
        i
      </button>
    </div>
